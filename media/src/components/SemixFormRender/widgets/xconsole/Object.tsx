    ...
    if (props.schema?.properties) {
      ...
      const renderField = (key: string) => {
        return (
          <Field
            ...
            schemaPath={getNextDataPath(
              props.schemaPath ? props.schemaPath + ".properties" : props.schemaPath,
              key
            )}
      ...
      const groups = _.map(propItemsGroups, (items, groupKey) => {
        if (groupKey) {
          ...
          const title = store.renderGroupArea?.(titleProps) || (
            <div className="area-title">{groupKey}</div>
          );
