...
// import "./SemixForm.scss";
import { getSchemaByRef, semixifySchema, usePrevious } from "./utils";
...
const InternalSemixForm: React.FC<SemixFormProps> = (props, ref) => {
  ...
  const renderTitle =
    props.renderTitle ||
    (widgets?.title ? (titleProps) => React.createElement(widgets?.title, titleProps) : null);
    ...
    return (
      <div className={classNames("semix-form", cx)}>
        <FormStoreContext.Provider initialState={formStoreState}>
          {props.children || (
            <Field
              schema={formStoreState.schema as any}
